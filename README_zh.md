...
* **轻量设置弹窗**：右下角悬浮按钮打开设置面板，轻松新增、编辑、删除快捷方式，并可随时查看浮层唤起快捷键。
...
| `↑` / `↓` | 搜索浮层 / 新标签页 | 跨类别平滑导航搜索结果 |
| `Enter` | 搜索浮层 / 新标签页 | 打开选中项 / 切换标签页 / 网址直达 / 执行网页搜索 |
| `Esc` | 搜索浮层 | 关闭浮层 |
| *右键点击瓦片* | 新标签页 | 呼出快捷方式管理菜单（编辑 / 删除 / 新标签页打开） |

> [!TIP]
> 如需修改唤起快捷键，可随时在 Chrome 地址栏访问 `chrome://extensions/shortcuts` 进行自定义配置。
...
| 权限声明 | 用途解释 |
| :--- | :--- |
| `storage` | 保存用户自定义的快捷方式列表及搜索选择频次统计（保存在 `chrome.storage.local`）。 |
