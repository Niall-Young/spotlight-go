...
* **轻量设置弹窗**：右下角悬浮按钮打开三段式设置面板——可选择主题（跟随系统 / 亮色 / 暗色）与界面语言（跟随系统 / 中文 / English）；点击「更改」后按下组合键即可自定义浮层唤起快捷键；快捷入口以卡片列表内联新增、编辑、删除；并可通过开关选择搜索来源（标签页 / 书签 / 历史记录）。
* **中英双语界面**：新标签页与搜索浮层完整支持中文 / English，默认跟随浏览器语言，也可在设置「通用」中手动切换。
